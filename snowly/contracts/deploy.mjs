// Puts the Snowly launchpad on Robinhood Chain. This launches no tokens — it
// deploys the snowfield, the fee hook and the glacier, and `launch.mjs` posts to it.
//
//   DEPLOYER_KEY=0x…    the key for the address snowly.config.json names
...
if (account.address.toLowerCase() !== intendedDeployer.toLowerCase()) {
  fail(
    `snowly.config.json expects to deploy from ${intendedDeployer}`,
...
const hash = await wallet.deployContract({
  ...
  bytecode: `0x${snowlyArtifact.evm.bytecode.object}`,
...
const read = (functionName) =>
  publicClient.readContract({ address: launchpad, abi: snowlyArtifact.abi, functionName });

const [hook, glacier] = await Promise.all([read("hook"), read("glacier")]);
...
recordDeployed(config, { launchpad, hook, glacier, hookSalt: mined.salt, deployedAt: new Date().toISOString() });

console.log(`\nNothing is launched yet. The snowfield is empty and anyone can post to it:\n`);
