...
const glacierArtifact = readArtifact("Glacier");
...
const [hook, glacier, launchpadTreasury] = await Promise.all([
  read(deployed.launchpad, snowlyArtifact.abi, "hook"),
  read(deployed.launchpad, snowlyArtifact.abi, "glacier"),
  read(deployed.launchpad, snowlyArtifact.abi, "treasury"),
]);

check("hook is the one in the config", hook.toLowerCase() === (deployed.hook ?? "").toLowerCase(), hook);
check("glacier is the one in the config", glacier.toLowerCase() === (deployed.glacier ?? "").toLowerCase(), glacier);

const [hookLaunchpad, glacierLaunchpad, hookTreasury] = await Promise.all([
  read(hook, hookArtifact.abi, "launchpad").catch(() => null),
  read(glacier, glacierArtifact.abi, "launchpad").catch(() => null),
...
check(
  "glacier answers only to this launchpad",
  glacierLaunchpad?.toLowerCase() === deployed.launchpad.toLowerCase(),
  glacierLaunchpad ?? "could not read",
);
...
const [supply, poolBps, walletBps, lpFee, feeBps, creatorBps, bps] = await Promise.all([
  read(deployed.launchpad, snowlyArtifact.abi, "FIXED_SUPPLY"),
  read(deployed.launchpad, snowlyArtifact.abi, "POOL_BPS"),
  read(deployed.launchpad, snowlyArtifact.abi, "SUPPLY_WALLET_BPS"),
  read(deployed.launchpad, snowlyArtifact.abi, "LP_FEE"),
  read(hook, hookArtifact.abi, "FEE_BPS"),
  read(hook, hookArtifact.abi, "CREATOR_BPS"),
  read(hook, hookArtifact.abi, "BPS"),
]);

const EXPECTED = { supply: 1_000_000_000n * 10n ** 18n, poolBps: 8_000n, walletBps: 2_000n, feeBps: 450n, creatorBps: 7_500n };
...
const name = process.env.NAME ?? "Snowly Test";
const symbol = process.env.SYMBOL ?? "TSNOW";
...
console.log("\nthe snowfield");
const driftCount = await read(deployed.launchpad, snowlyArtifact.abi, "driftCount");
console.log(`  -- ${"drifts already launched".padEnd(44)} ${driftCount}`);
if (driftCount === 0n) console.log(`  -- ${"".padEnd(44)} the next launch becomes drift #0, permanently`);
