...
const CONFIG_PATH = join(dirname(dirname(fileURLToPath(import.meta.url))), "snowly.config.json");
...
export function loadConfig() {
  ...
  try {
    ...
    fail(`cannot read snowly.config.json: ${error.message}`);
  }
...
function refuseSecrets(node, path) {
  if (typeof node === "string") {
    if (!PUBLIC_32_BYTE_VALUES.has(path) && /^0x[0-9a-fA-F]{64}$/.test(node.trim())) {
      fail(
        `${path || "a value"} in snowly.config.json looks like a private key.`,
        "",
...
export function configAddress(config, path, envName, { what }) {
  ...
  if (value === "") {
    fail(
      `${path} is not set in snowly.config.json — ${what}`,
      "",
...
export function configPrice(config, path, envName, { what }) {
  ...
  if (value === "") fail(`${path} is not set in snowly.config.json — ${what}`);
  return value;
}
...
export function recordDeployed(config, values) {
  ...
  saveConfig(config);
  console.log(`\nwritten to snowly.config.json: ${Object.keys(values).join(", ")}`);
}
