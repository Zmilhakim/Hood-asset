...
//
// SnowHook checks this in its own constructor and reverts when it is wrong, so
...
/**
 * The five callbacks SnowHook asks for, and nothing else.
 *
 ...
 *
 * The same value is `REQUIRED_FLAGS` in SnowHook.sol. If one changes, both do.
 */
export const SNOW_HOOK_FLAGS =
...
export function hasFlags(address, flags = SNOW_HOOK_FLAGS) {
  return (BigInt(address) & ALL_HOOK_MASK) === flags;
}

/** SnowHook's deployment bytecode: its creation code, then its two arguments. */
 ...
 * below is how.
 */
export function mineHookSalt({ deployer, initCode, flags = SNOW_HOOK_FLAGS, limit = 2_000_000 }) {
