...
console.log(`grate      ${mined.address}`);
console.log(`salt       ${mined.salt}   (found in ${mined.tries} tries)`);
console.log(`flags      ${flagsOf(mined.address).join(", ")}`);
// The sump is plain CREATE from the launchpad. Its nonce is 2, not 1: a contract
...
const sump = getContractAddress({ from: launchpad, nonce: 2n });
...
for (const [label, address] of [["launchpad", launchpad], ["grate", mined.address], ["sump", sump]]) {
