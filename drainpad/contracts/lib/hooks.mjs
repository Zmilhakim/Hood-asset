...
//
// Grate checks this in its own constructor and reverts when it is wrong, so
...
/**
 * The five callbacks Grate asks for, and nothing else.
 *
 ...
 *
 * The same value is `REQUIRED_FLAGS` in Grate.sol. If one changes, both do.
 */
export const GRATE_FLAGS =
...
export function hasFlags(address, flags = GRATE_FLAGS) {
  return (BigInt(address) & ALL_HOOK_MASK) === flags;
}

/** Grate's deployment bytecode: its creation code, then its two arguments. */
 ...
 * below is how.
 */
export function mineHookSalt({ deployer, initCode, flags = GRATE_FLAGS, limit = 2_000_000 }) {
