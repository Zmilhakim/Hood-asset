...
const CONFIG_PATH = join(dirname(dirname(fileURLToPath(import.meta.url))), "drainpad.config.json");
...
export function loadConfig() {
  ...
  try {
    ...
    fail(`cannot read drainpad.config.json: ${error.message}`);
  }
...
function refuseSecrets(node, path) {
  if (typeof node === "string") {
    if (!PUBLIC_32_BYTE_VALUES.has(path) && /^0x[0-9a-fA-F]{64}$/.test(node.trim())) {
      fail(
        `${path || "a value"} in drainpad.config.json looks like a private key.`,
        "",
...
export function configAddress(config, path, envName, { what }) {
  ...
  if (value === "") {
    fail(
      `${path} is not set in drainpad.config.json — ${what}`,
      "",
...
export function configPrice(config, path, envName, { what }) {
  ...
  if (value === "") fail(`${path} is not set in drainpad.config.json — ${what}`);
  return value;
}
...
export function recordDeployed(config, values) {
  ...
  saveConfig(config);
  console.log(`\nwritten to drainpad.config.json: ${Object.keys(values).join(", ")}`);
}
