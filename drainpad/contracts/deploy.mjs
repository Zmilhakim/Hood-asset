// Puts the Drainpad launchpad on Robinhood Chain. This launches no tokens — it
// deploys the catchment, the fee hook and the sump, and `launch.mjs` posts to it.
//
//   DEPLOYER_KEY=0x…    the key for the address drainpad.config.json names
...
if (account.address.toLowerCase() !== intendedDeployer.toLowerCase()) {
  fail(
    `drainpad.config.json expects to deploy from ${intendedDeployer}`,
...
console.log(`grate      ${mined.address} (${flagsOf(mined.address).join(", ")})`);
...
const read = (functionName) =>
  publicClient.readContract({ address: launchpad, abi: drainpadArtifact.abi, functionName });

const [hook, sump] = await Promise.all([read("grate"), read("sump")]);
...
console.log(`\nlaunchpad  ${launchpad}`);
console.log(`grate      ${hook} (flags confirmed on chain)`);
...
recordDeployed(config, { launchpad, hook, sump, hookSalt: mined.salt, deployedAt: new Date().toISOString() });

console.log(`\nNothing is launched yet. The catchment is empty and anyone can post to it:\n`);
