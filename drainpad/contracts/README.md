...
EVM against Uniswap's own `PoolManager`, deployed exactly as it ships. No
Hardhat, no Foundry, and no network access needed to run the tests.

| Contract | Job |
| --- | --- |
| `Drainpad` | The catchment. Launches a token, splits the supply, opens the pool, sinks the pool's share — one transaction. |
| `Grate` | 4% of everything paid into a pool, either direction. 75% to the creator, 25% to the treasury. |
| `Sump` | Holds every launch's liquidity, and has no function that hands any back. |
| `DrainToken` | Fixed-supply ERC20, printed once to exactly two addresses. |
...
npm run mine      # the salt the grate needs, printed before anything is spent
npm run deploy    # puts the launchpad, the grate and the sump on chain
npm run launch    # puts one token in the catchment — prints the plan first
npm run status    # what the pool manager says about the catchment, right now
...
1. Deploys the token, whose constructor prints the entire fixed supply —
   1,000,000,000 — and splits it in that same constructor: **750,000,000 to the
   sump, 250,000,000 to the supply wallet**.
2. Opens a v4 pool of **native ETH against the token**, with the `Grate` in the
   key and an **LP fee of zero**, priced at the top of the launch range.
3. Sinks the sump's 750,000,000 in as one position the sump cannot pump back
   out.
...
contract here restrains it and none of them pretends to — `DrainToken` has no
...
token — `POOL_BPS` and `SUPPLY_WALLET_BPS` in `Drainpad.sol`.
...
pay a seller with. It becomes sellable as the pool fills.
`test/contracts.test.mjs` asserts both halves of that.

## The fee

4% of everything paid into a pool, in either direction. Buy with ETH and the 4%
is kept in ETH; sell the token back and it is kept in the token. 75% goes to
whoever launched it, 25% to the treasury, and both figures are `constant` with
no setter.

The rate lives in the grate, and a pool's hook is part of its key — so it is
fixed when the pool is opened. Not governed, not timelocked: a different hook is
a different pool. Drainpad pools carry an LP fee of zero, so this 4% is the
...
lifted as cash mid-swap. Lifting cash at that point would revert on a pool with
no ETH in it yet, which is every pool until its first buy. `withdraw` redeems
the claims later, in a transaction of its own.

## The range

The pool opens at the **floor** and the range tops out at the **ceiling**, both
quoted in `drainpad.config.json` as market caps for the whole supply rather than
prices per token, because that is how anyone actually thinks about a launch. The
shipped range is **1.8 ETH to 180 ETH** — a hundredfold.
...
The whole of the sump's share goes in below spot, and spot starts at the top of
the range: the point where the token is cheapest, so the first buy fills
immediately and the pool never asks the sump for ETH it does not have. The
launchpad enforces that — a range reaching above spot reverts.

## The grate's address is mined for

A v4 pool works out which callbacks to make by reading the low 14 bits of the
hook's own address, so the permissions *are* the address. `Grate` cannot be
deployed wherever it lands: `Drainpad`'s constructor deploys it with CREATE2
from a salt mined off-chain, and the grate's own constructor checks the result
...
useful message. That includes the grate's CREATE2 salt, which is why
...
**Launched tokens are verified too.** `DrainToken` is deployed fresh by every
launch with six constructor arguments, and none of them is stored anywhere off
chain — they are read back out of the runoff the launchpad recorded: the name,
the ticker, the sump, its share, the supply wallet and its share. So a token
...
`npm run verify` does the launchpad, the grate, the sump and every token in the
catchment. `npm run verify -- --id 3` does one launched token and nothing else.
...
`drainpad.config.json` holds addresses and two prices. Private keys never go in
