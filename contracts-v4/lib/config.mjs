// The board, written down once.
//
// Everything here is public: contract addresses, a fee tier, two prices. The
...
// immutable once the board is deployed, so the difference between the right address and a
...
const CONFIG_PATH = join(dirname(dirname(fileURLToPath(import.meta.url))), "hoodpad.config.json");
...
export function loadConfig() {
  ...
  try {
    ...
    fail(`cannot read hoodpad.config.json: ${error.message}`);
  }
...
function refuseSecrets(node, path) {
  if (typeof node === "string") {
    if (/^0x[0-9a-fA-F]{64}$/.test(node.trim())) {
      fail(
        `${path || "a value"} in hoodpad.config.json looks like a private key.`,
        "",
...
export function configAddress(config, path, envName, { what }) {
  ...
  if (value === "") {
    fail(
      `${path} is not set in hoodpad.config.json — ${what}`,
      "",
...
export function configPrice(config, path, envName, { what }) {
  ...
  if (value === "") fail(`${path} is not set in hoodpad.config.json — ${what}`);
  return value;
}
...
/** Records what a deploy produced, so the next step needs no copying. */
export function recordDeployed(config, values) {
  config.deployed = { ...config.deployed, ...values };
  saveConfig(config);
  console.log(`\nwritten to hoodpad.config.json: ${Object.keys(values).join(", ")}`);
}
