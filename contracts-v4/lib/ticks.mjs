...
// turns "the whole supply is worth this much ETH" into the numbers `postToken`
// takes.
//
// getSqrtPriceAtTick is Uniswap v4's TickMath, transliterated rather than
// approximated. That matters: a launch initialises the pool at exactly the top
// of the range, and a price one tick off would be a launch the factory refuses.
...
/**
 * The launch range for a launch priced in ETH.
 *
 * In v4 the other side of the pool is native ETH, which is address zero and so
 * is always currency0. the token is therefore always currency1 — there is no
 * ordering to discover — and a pool prices currency1 in currency0, which here
 * means the token per ETH. That runs the opposite way to the price being quoted: a
 ...
 * the point where the token is cheapest, so the first buy fills immediately and the
...
export function launchRange({ floorEthPerToken, ceilEthPerToken, tickSpacing }) {
  ...
  // Inverted on the way in, because the pool counts the token per ETH.
 ...
 * into the launch and cannot come out, and `tokens` is what is still on the
 * shelf — the two figures worth putting in front of anyone.
 *
 * The launch opens with the price above its whole range, which is the same thing
 * as saying the position is all the token and holds no ETH at all. So `eth` of zero
 ...
 * therefore always currency0 — so what the pool quotes is the token per ETH, and
...
/**
 * The whole plan for one launch, from the two market caps a poster actually
 * thinks in.
 *
 * Hoodpad quotes a launch the way people talk about one — "it opens at 1 ETH and
 * the range runs to 100" — where both numbers value the *entire supply*. The
 * pool wants a price per token and a pair of aligned ticks, and this is the only
 * place that conversion happens: the web app, the launch script and the tests
 * all import it rather than each doing their own.
 *
 * @param openingEth market cap in ETH where the sale starts. The pool opens here.
 * @param ceilingEth market cap in ETH at the far end of the range.
 * @param tickSpacing the spacing the pool key will carry.
 * @param wholeSupply tokens, not wei — 1_000_000_000n, not 1e27.
 */
export function planLaunch({ openingEth, ceilingEth, tickSpacing, wholeSupply = 1_000_000_000n }) {
  const range = launchRange({
    floorEthPerToken: pricePerToken(String(openingEth), wholeSupply),
    ceilEthPerToken: pricePerToken(String(ceilingEth), wholeSupply),
    tickSpacing,
  });

  // What the ticks actually landed on, which is not quite what was asked for —
  // ticks are discrete and spacing is coarse. Reported rather than rounded away,
  // so a launch script can print the market cap the pool will really open at.
  const capAtTick = (tick) => {
    const sqrtPrice = getSqrtPriceAtTick(tick);
    return Number((ethPerTokenFromSqrtPrice(sqrtPrice) * wholeSupply) / 10n ** 18n);
  };

  return {
    ...range,
    effectiveOpeningCapEth: capAtTick(range.tickUpper),
    effectiveCeilingCapEth: capAtTick(range.tickLower),
  };
}
