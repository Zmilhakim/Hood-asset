 ...
 *
 * The site needs it for one thing: turning the position's tick bounds into
 * prices, so the ETH sitting inside the crate can be worked out from the
 * liquidity figure the manager reports.
 */
 ...
 *
 * This is the figure worth showing as "liquidity": not an abstract L, but the
 * ETH that has gone into the crate and cannot come out. Above the range the
 * position is still all token and holds none; below it, it holds the lot.
 *
...
export function ethInPosition(liquidity: bigint, sqrtPriceX96: bigint, tickLower: number, tickUpper: number): bigint {
  ...
  // Outside the range the position is entirely one currency, so clamping is
