...
import {ClayHook} from "./ClayHook.sol";
import {ClayToken} from "./ClayToken.sol";
import {Kiln} from "./Kiln.sol";

/// @title Clayspad
/// @notice The shelf. Every token launched through Clayspad is recorded here,
/// and every figure the site prints is read back out of this contract or the
/// pool manager. Nothing is estimated off-chain.
///
/// A launch is one transaction and costs nothing but gas:
///
///   1. mint the whole fixed supply, split in the token's own constructor —
///      75% to the kiln, 25% to the supply wallet,
///   2. open a Uniswap v4 pool of native ETH against the token, with `ClayHook`
///      in the key and an LP fee of zero,
///   3. fire the kiln's 75% into the pool as one position it can never take out.
///
/// ## The split, stated plainly
///
/// Three quarters of every supply goes into the pool and does not come back.
/// The remaining quarter is minted straight to a wallet the creator nominates,
/// and it is **liquid immediately** — not vested, not cliffed, not locked. No
/// contract in this repository restrains it, and none of them pretends to.
///
...
/// split**, for the reason above. **The LP fee, at zero**, so the 4% in the
...
contract Clayspad {
    ...
    /// @notice Clayspad pools charge no LP fee. The 4% in `ClayHook` is the
    /// entire fee schedule.
    uint24 public constant LP_FEE = 0;

    IPoolManager public immutable poolManager;
    ClayHook public immutable hook;
    ...
    Piece[] private _pieces;
    mapping(address creator => uint256[] pieceIds) private _piecesOf;
    mapping(PoolId poolId => uint256 pieceId) private _pieceOfPool;
    ...
    event SupplyFired(uint256 indexed id, PoolId indexed poolId, int24 tickLower, int24 tickUpper, uint128 liquidity);
    ...
    constructor(IPoolManager poolManager_, address treasury_, bytes32 hookSalt) {
        ...
        treasury = treasury_;
        hook = new ClayHook{salt: hookSalt}(poolManager_, treasury_);
        kiln = new Kiln(poolManager_);
    }

    // --------------------------------------------------------------- launching

    /// @notice Launch a token, open its pool, and fire the pool's share into it.
        ...
        returns (uint256 id, address token, PoolId poolId, uint128 liquidity)
    {
        ...
        // path to address(0) here: a quarter of a supply minted to nowhere would
        ...
        poolId = key.toId();
        liquidity = _openAndFire(key, params);
    ...
    /// the pool's three quarters to the kiln, the remaining quarter to the
    /// supply wallet. Neither share ever passes through this contract.
    function _mintToken(LaunchParams calldata params, address supplyWallet) private returns (address) {
        ...
        return address(
            new ClayToken(params.name, params.symbol, address(kiln), toPool, supplyWallet, toSupplyWallet)
        );
    }

    /// @dev Registers the pool with the hook, initialises it, and fires the
    /// kiln's share in — the three steps that touch the pool manager.
    function _openAndFire(PoolKey memory key, LaunchParams calldata params) private returns (uint128) {
        ...
        // need ETH as well, and the kiln has none to give — that would fail
        // inside the pool manager's callback, which is a worse place to find out.
        if (params.tickUpper > tick) revert NotSingleSided();

        return kiln.fire(key, params.tickLower, params.tickUpper);
    }

    /// @dev Writes the piece to the shelf and emits what a reader indexes on.
    function _record(
        ...
        _piecesOf[msg.sender].push(id);
        _pieceOfPool[poolId] = id + 1; // +1, so "no piece" and "piece zero" differ
        ...
        emit SupplyFired(id, poolId, params.tickLower, params.tickUpper, liquidity);
    }
    ...
    function pieceAt(uint256 id) external view returns (Piece memory) {
        return _pieces[id];
    }

    /// @notice A page of the shelf, newest first — the order the feed reads in.
    function latest(uint256 offset, uint256 limit) external view returns (Piece[] memory page) {
        uint256 total = _pieces.length;
        if (offset >= total) return new Piece[](0);
        ...
        for (uint256 i = 0; i < size; i++) {
            page[i] = _pieces[total - 1 - offset - i];
        }
    }

    function piecesOf(address creator) external view returns (uint256[] memory) {
        return _piecesOf[creator];
    }

    /// @notice The piece a pool belongs to. Reverts for a pool this contract did
    /// not open, rather than answering about piece zero.
    function pieceOfPool(PoolId poolId) external view returns (Piece memory) {
        uint256 slot = _pieceOfPool[poolId];
        if (slot == 0) revert UnknownPool();
        return _pieces[slot - 1];
    }

    /// @notice The pool key Clayspad opens for a token: native ETH against it,
    /// no LP fee, this hook in the key.
    ...
    /// token always `currency1`. `fee` is zero because the hook's 4% is the whole
    /// schedule. And `hooks` is part of the key, so a pool with a different hook
    /// in it is not this pool with the rate changed — it is a different pool.
    ...
    /// @notice The pool key for a piece — everything needed to trade it, or to
    /// read it out of the pool manager directly.
    function poolKeyOf(uint256 id) public view returns (PoolKey memory) {
        Piece memory piece = _pieces[id];
        return poolKeyFor(piece.token, piece.tickSpacing);
    }
    ...
    /// @notice Everything the shelf header needs, in one call.
    function shelfStats()
        external
        view
        returns (
