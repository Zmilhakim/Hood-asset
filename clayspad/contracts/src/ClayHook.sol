...
/// @title ClayHook
/// @notice Every Clayspad pool charges **4% of everything paid into it**, in
/// either direction. Buy with ETH and the 4% is taken in ETH; sell the token
...
///
/// There is no second fee. Clayspad pools are opened with an LP fee of zero, so
...
///     of what the curve charged, at 4/96 of it, which leaves the fee at 4% of
...
/// every Clayspad pool is until its first buy — that reverts, and the launch
...
contract ClayHook is IHooks, IUnlockCallback {
    ...
    /// @notice The fee: 4% of everything paid into a Clayspad pool.
    ...
    /// @param poolManager_ The Uniswap v4 pool manager every Clayspad pool lives in.
    ...
    /// with this hook in its key, so nobody can point this fee at a pool
    /// Clayspad did not launch — and every pool that exists has a creator on
    /// file, so a fee can never be collected with nowhere to send it.
        ...
        returns (bytes4, int128)
    {
        ...
        // 4/96 of what the curve charged, so the fee is still 4% of the total
        // the trader parts with. Rounded up, so rounding is never a discount.
