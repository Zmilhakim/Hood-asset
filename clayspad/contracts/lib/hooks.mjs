...
//
// ClayHook checks this in its own constructor and reverts when it is wrong, so
...
/**
 * The five callbacks ClayHook asks for, and nothing else.
 *
 * `beforeInitialize` is how only the launchpad can open a pool with this hook in
 ...
 *
 * The same value is `REQUIRED_FLAGS` in ClayHook.sol. If one changes, both do.
 */
export const CLAY_HOOK_FLAGS =
...
export function hasFlags(address, flags = CLAY_HOOK_FLAGS) {
  return (BigInt(address) & ALL_HOOK_MASK) === flags;
}

/** ClayHook's deployment bytecode: its creation code, then its two arguments. */
 ...
 * proof of work — it takes well under a second. `deployer` is the launchpad,
 * because the launchpad is what runs the CREATE2, and the launchpad's own address
 * therefore has to be known before this can be answered. `predictLaunchpad`
 * below is how.
 */
export function mineHookSalt({ deployer, initCode, flags = CLAY_HOOK_FLAGS, limit = 2_000_000 }) {
...
/**
 * Where the launchpad will land if `deployer` sends the next transaction from
 * `nonce`.
 *
 * The hook's address depends on the launchpad's, and the launchpad deploys the hook
 ...
 */
export function predictLaunchpad({ deployer, nonce }) {
