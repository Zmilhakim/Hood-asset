...
const CONFIG_PATH = join(dirname(dirname(fileURLToPath(import.meta.url))), "clayspad.config.json");
...
export function loadConfig() {
  ...
  try {
    ...
    fail(`cannot read clayspad.config.json: ${error.message}`);
  }
...
export function configAddress(config, path, envName, { what }) {
  ...
  if (value === "") {
    fail(
      `${path} is not set in clayspad.config.json — ${what}`,
      "",
...
export function configPrice(config, path, envName, { what }) {
  ...
  if (value === "") fail(`${path} is not set in clayspad.config.json — ${what}`);
  return value;
}
...
export function recordDeployed(config, values) {
  ...
  saveConfig(config);
  console.log(`\nwritten to clayspad.config.json: ${Object.keys(values).join(", ")}`);
}
