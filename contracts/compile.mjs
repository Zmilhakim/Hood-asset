...
// Fixtures are test-only stand-ins (a WETH9 the local EVM can actually call).
// They go through the same compiler settings as everything else, but they are
// never exported to the web app and never deployed.
const input = {
  language: "Solidity",
  sources: { ...collectSources(srcDir), ...collectSources(fixtureDir, "fixtures") },
  settings: {
...
const FIXTURES = {
  "fixtures/WETH9.sol": "WETH9",
};

for (const [file, name] of Object.entries(FIXTURES)) {
  writeFileSync(join(outDir, `${name}.json`), JSON.stringify(output.contracts[file][name], null, 2));
  console.log(`${name.padEnd(16)} ${"".padStart(6)}                  test fixture`);
}
