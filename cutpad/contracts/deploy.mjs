// Puts the Cutpad factory on Robinhood Chain. This launches no tokens — it
// deploys the board, the cut hook and the locker, and `launch.mjs` posts to it.
//
//   DEPLOYER_KEY=0x…    the key for the address cutpad.config.json names
...
const factoryArtifact = readArtifact("CutpadFactory");
const hookArtifact = readArtifact("CutHook");
...
const treasury = configAddress(config, "treasury", "TREASURY", {
  what: "where the treasury's share of every cut goes, forever",
});
...
if (account.address.toLowerCase() !== intendedDeployer.toLowerCase()) {
  fail(
    `cutpad.config.json expects to deploy from ${intendedDeployer}`,
...
if (chain.id !== config.chainId) fail(`cutpad.config.json says chain ${config.chainId}, not ${chain.id}`);
...
// anywhere that changes it, so a typo here is a typo forever — every cut the
