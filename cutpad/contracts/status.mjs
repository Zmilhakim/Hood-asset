...
import { readSlot0, cutpadPoolKey, poolId } from "./lib/pool.mjs";
...
const factory = configAddress(config, "deployed.factory", "FACTORY", {
  what: "the Cutpad factory — run `npm run deploy` first",
});
...
if (publicClient.chain.id !== config.chainId) {
  fail(`cutpad.config.json says chain ${config.chainId}, not ${publicClient.chain.id}`);
}
...
const [tokens, lastLaunch, supply, cutBps, creatorBps] = stats;

console.log(`factory    ${factory}`);
console.log(`hook       ${hook}`);
console.log(`locker     ${locker}`);
console.log(`\nlaunches   ${tokens}`);
console.log(`cut       ${Number(cutBps) / 100}% of every swap — ${Number(creatorBps) / 100}% of it to the creator`);
...
for (const notice of page) {
  const key = cutpadPoolKey({ token: notice.token, hook, tickSpacing: notice.tickSpacing });
