...
| Contract | Job |
| --- | --- |
| `CutpadFactory` | The board. Launches a token, opens its pool, locks the supply in — one transaction. |
| `CutHook` | The cut gate. 3% of every swap, 80% to the creator, 20% to the treasury. |
| `CutLocker` | Holds every launch's liquidity, and has no function that gives any back. |
| `CutToken` | Fixed-supply ERC20. No mint, no owner, no pause. |
...
npm run collect   # take the cut you are owed
```
...
1. Deploys the token and mints the entire fixed supply — 1,000,000,000 — straight
   to the locker.
2. Opens a v4 pool of **native ETH against the token**, with `CutHook` in the
...
**3% of everything paid into the pool, in either direction.** Buy with ETH and
the cut is 3% of the ETH. Sell the token back and it is 3% of the token. Of
...
Charging the input means a buy pays its cut in ETH; charging the output would
...
* **Exact input** (`amountSpecified < 0`) — the specified currency is the input.
  ...
  asked to pay and 97% of it is swapped.
* **Exact output** (`amountSpecified > 0`) — the specified currency is the
  output, and the input is not known until the curve has run. So the cut is
  charged in `afterSwap`, whose return lands on the unspecified currency — the
  input — at 3/97 of what the swap cost, rounded up. Still 3% of the total.

There is a test for each, and one for a swap so small the cut rounds to zero.
...
whose cut is quietly never charged is the failure mode worth spending a
...
open a pool with this hook in it. Nobody can point the cut at a pool Cutpad did
not launch, and no pool can exist with a cut and nowhere to send it.
...
`modifyLiquidity` with a negative delta. There is no such call in `CutLocker`.
...
**The cut does come out**, to two addresses fixed before the token existed: the
...
| LP fee, at zero | So the cut is the only fee, and no fee accrues where nobody can collect it |
...
Everything public lives in `cutpad.config.json`, and every script reads it from
there:
...
`cutpad.config.json` has no addresses under `deployed` because nothing has been
