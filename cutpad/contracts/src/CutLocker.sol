...
/// @title CutLocker
/// @notice Holds the liquidity of every token launched through Cutpad, and has
...
/// deployment less.
///
/// There is nothing to collect here either. Cutpad pools are opened with an LP
/// fee of zero — the 3% cut in `CutHook` is the entire fee schedule — so this
/// contract never accrues fees that would then need a way out.
contract CutLocker is IUnlockCallback {
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        // The token is currency1 in every Cutpad pool: the other side is native
        ...
        if (dust > 0) CutToken(Currency.unwrap(key.currency1)).burn(dust);
