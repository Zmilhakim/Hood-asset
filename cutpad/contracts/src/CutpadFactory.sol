...
import {CutHook} from "./CutHook.sol";
import {CutLocker} from "./CutLocker.sol";
import {CutToken} from "./CutToken.sol";

/// @title CutpadFactory
/// @notice The board. Every token launched through Cutpad is recorded here, and
...
///   2. open a Uniswap v4 pool of native ETH against it, with `CutHook` in the
...
/// through their hands or this contract's — and owning one thing: 80% of the 3%
/// cut that pool charges from its first trade onwards, forever.
///
/// ## What is fixed, and why
///
/// **The supply**, so no launch can quietly print more than another. **The LP
/// fee, at zero**, so the cut is the only fee anyone has to reason about and
...
contract CutpadFactory {
    ...
    /// @notice Cutpad pools charge no LP fee. The 3% cut in `CutHook` is the
    /// entire fee schedule.
    uint24 public constant LP_FEE = 0;

    IPoolManager public immutable poolManager;
    CutHook public immutable hook;
    CutLocker public immutable locker;

    /// @notice Where the treasury's share of every cut goes. Recorded here for
    ...
    /// @param treasury_ Where the treasury's 20% of every cut goes, forever.
    ...
    constructor(IPoolManager poolManager_, address treasury_, bytes32 hookSalt) {
        ...
        treasury = treasury_;
        hook = new CutHook{salt: hookSalt}(poolManager_, treasury_);
        locker = new CutLocker(poolManager_);
    }
        ...
        returns (uint256 id, address token, PoolId poolId, uint128 liquidity)
    {
        ...
        token = address(new CutToken(params.name, params.symbol, FIXED_SUPPLY, address(locker)));
    ...
    function boardStats()
        external
        view
        returns (uint256 tokens, uint256 lastLaunch, uint256 supply, uint256 cutBps, uint256 creatorBps)
    {
        return (_notices.length, lastLaunchAt, FIXED_SUPPLY, hook.CUT_BPS(), hook.CREATOR_BPS());
    }
}
