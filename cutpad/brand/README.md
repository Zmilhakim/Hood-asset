...
| `logo-wordmark.svg` | CUTPAD, no mark |
...
| `cut-1600x900.png` | The fee schedule, as one card |
...
`lib/marks.mjs` draws it, and every letter of CUTPAD, as rectangles. A logo set
...
`render.mjs` reads `CUT_BPS`, `CREATOR_BPS`, `LP_FEE` and `FIXED_SUPPLY` out of
`../contracts/src/` and throws if they are not what the copy says. It also greps
`CutLocker.sol` for a `withdraw`, a `collect`, a `rescue` or a negative
liquidity delta, and throws if it finds one.

A card printing *3%* and *locked permanently* is a claim about deployed code.
