...
export const BRAND = {
  ...
  venue: "UNISWAP V4",
  promise: "3% CUT · 80% TO THE CREATOR",
  tagline: "LAUNCH A TOKEN. CHARGE A CUT.",
  line: "One hook, one fee, both directions. The supply is all in the pool and the pool is locked — what a launch earns is the cut.",
};
 ...
 *
 * A card that prints "3%" is a claim about a deployed contract, and it is worth
...
const CUT_BPS = Number(constantFrom("CutHook.sol", "CUT_BPS"));
const CREATOR_BPS = Number(constantFrom("CutHook.sol", "CREATOR_BPS"));
const SUPPLY = constantFrom("CutpadFactory.sol", "FIXED_SUPPLY");
const LP_FEE = Number(constantFrom("CutpadFactory.sol", "LP_FEE"));
...
if (CUT_BPS !== 500 || CREATOR_BPS !== 8_000 || SUPPLY !== 1_000_000_000n * 10n ** 18n) {
  throw new Error(`the contracts now say cut ${CUT_BPS}bps, creator ${CREATOR_BPS}bps, supply ${SUPPLY} — rewrite the copy before re-rendering`);
}
 ...
 * made it true.
 */
const lockerSource = readFileSync(join(contracts, "CutLocker.sol"), "utf8");
...
for (const pattern of FORBIDDEN) {
  if (pattern.test(lockerSource)) {
    throw new Error(`CutLocker.sol now matches ${pattern} — the lock card would be a lie`);
  }
}
// ---------------------------------------------------------------------------
...
function cutTicket({ width }) {
  ...
  <div class="ticket" style="width:${width}px">
    <div style="display:flex;justify-content:space-between;background:${PALETTE.signal};color:${PALETTE.ink};padding:9px 18px;font-size:12px;font-weight:600;letter-spacing:.18em">
      <span>CUT TICKET</span><span>${BRAND.venue}</span>
    </div>
    <div class="row"><span>Every swap</span><b>${RATE.cut}</b></div>
    ...
    <div style="flex:1;display:flex;align-items:center;gap:54px;padding:0 60px">
      <div style="flex:1">
        ...
        <div style="margin-top:22px;display:flex;gap:10px">
          <span class="chip solid">${RATE.cut} CUT</span>
  ...
  <div class="frame asphalt">
    <div class="panel">
      ...
      <div style="flex:1;min-height:0;padding:30px 46px;display:flex;flex-direction:column;justify-content:center">
        ...
        <h1 class="display" style="font-size:58px;line-height:1.08;margin-top:24px;max-width:15ch;color:${PALETTE.paper}">Every swap pays a cut</h1>
        <p style="margin-top:18px;font-size:19px;line-height:1.55;color:${PALETTE.inkFaint};max-width:60ch">
          ${RATE.cut} of everything paid into the pool, in either direction. ${RATE.creator} of it goes to whoever launched the token, and the rate is in the pool's key — so it is the same on the last day as on the first.
        </p>
...
const cutCard = `<!doctype html><html><head><meta charset="utf-8">${FONTS}<style>${BASE}
    ...
    <div class="panel">
      ...
      <div style="flex:1;min-height:0;display:flex;align-items:center;gap:56px;padding:36px 48px">
        <div style="flex:0 0 auto;display:flex;flex-direction:column;align-items:center;gap:22px">
          ${gateSvg({ size: 240 })}
          <div class="display" style="font-size:78px;line-height:1;color:${PALETTE.signal}">${RATE.cut}</div>
          <div class="micro" style="font-size:16px">OF EVERY SWAP</div>
        </div>
        <div style="flex:1;min-width:0">
          <div class="split"><span>To the creator</span><b>${RATE.creator} of the cut</b></div>
          <div class="split"><span>To the treasury</span><b>${RATE.treasury} of the cut</b></div>
...
for (const { name, html, size, faces } of [
  ...
  { name: "cut-1600x900", html: cutCard, size: { width: 1600, height: 900 }, faces: ["Archivo Black", "IBM Plex Mono"] },
]) {
  ...
  const missing = await tab.evaluate(async (families) => {
    ...
    const measure = (stack) => {
      ...
      el.textContent = "Every swap pays a cut";
...
console.log(`brand kit written to out/ — cut ${RATE.cut}, creator ${RATE.creator}, checked against the contracts`);
